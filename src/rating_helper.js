...
function rating_to_points(rating) {
	let p = 60 / (1 + Math.exp(-(rating-2300)/600));
...
function update_rating(old_rating, rating_deviation, points) {
	let perf_rating = points_to_rating(Math.abs(points));
	let delta = perf_rating - old_rating;
	if (delta < -100 && points > 0) {
	...
	let new_rating_deviation = Math.min(Math.round(Math.sqrt((rating_deviation + 1) * (Math.abs(delta) + 1) - 1)), 350);
